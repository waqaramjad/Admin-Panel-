...
import { getAddsDataBySt } from '../../store/action/action'
import { apps } from 'firebase';

class JobsSt extends Component {
    constructor(props) {
                    ...
                    Object.keys(studentData).map(function (key, index) {
                        ...
                        return (
                            ...
                            <div key={index} className="container">
                                <h2 className="list-group-item active">Job : {studentData[key].Job}</h2>
                                <div className="list-group">

                                    <h6 className="list-group-item">discrition :{studentData[key].discrition}</h6>
                                    <h6 className="list-group-item">email :{studentData[key].email}</h6>
                                    <h6 className="list-group-item">sallary :{studentData[key].sallry}</h6>
...
function mapStateToProp(state) {
    return ({
        studentData: state.root.getAddsDataBySt
    })
}
...
function mapDispatchToProp(dispatch) {
    ...
    return ({
        ...
        getDataFromStudent: (user) => {
            dispatch(getAddsDataBySt(user))
        }
    })
}

// export default Company;
export default connect(mapStateToProp, mapDispatchToProp)(JobsSt);